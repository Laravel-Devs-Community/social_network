<aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-52 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
    <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
        <ul class="space-y-2 font-medium">
            <li>
                <x-sidebar-item route="{{ 'home' }}" label="{{ __('Home') }}">
            ...
            <li>
                <x-sidebar-item route="{{ 'messages' }}" label="{{ __('Messages') }}">
                    <span class="inline-flex justify-center items-center ml-4">
            ...
            <li>
                <x-sidebar-item route="{{ 'friends' }}" label="{{ __('Friends') }}">
                    <span class="inline-flex justify-center items-center ml-4">
            ...
            <li>
                <x-sidebar-item route="{{ 'profile.show' }}" label="{{ __('Profile') }}">
                    <span class="inline-flex justify-center items-center ml-4">
            ...
            <li>
                <form method="POST" action="{{ route('logout') }}" x-data class="w-full">
                    @csrf
                    <a href="{{ route('logout') }}" @click.prevent="$root.submit();" class="relative flex flex-row items-center h-11 focus:outline-none hover:bg-gray-100 text-gray-600 hover:text-gray-800 border-l-4 border-transparent 
                    {{-- hover:border-indigo-500 --}}
                     pr-6 {{ request()->routeIs('logout') ? 'bg-white' :'' }} rounded-xl">
                        <span class="inline-flex justify-center items-center ml-4">
